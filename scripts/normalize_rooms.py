...
def rewrite_h1_in_markdown(md_path: Path, canonical_title: str) -> bool:
    ...
    # No H1 found: insert at top (skip leading blank lines)
    insert_at = 0
...
EXIT_TOKEN_RE = re.compile(
    r"""^\s*
    (?P<prefix>\([^)]*\)\s*)?                                            # optional leading "(...) "
    (?P<token>NE|NW|SE|SW|N|S|E|W|U|D|WAIT|LAND|LAUNCH)                   # token (longest first)
    (?:/(?P<token2>NE|NW|SE|SW|N|S|E|W|U|D))?                             # optional compound like "N/U"
    (?P<inline>\s*\([^)]*\)\s*)?                                         # optional inline "(...)" like "LAND (on west shore)"
    (?P<colon>\s*:)?\s*                                                  # optional colon like "N (condition):"
    →\s*
    (?P<link>\[\[Z1\s*-\s*[^\]]+\]\])                                    # IMPORTANT: spaces must be explicit in VERBOSE
...
def normalize_room_markdown(md_path: Path, *, fix_titles: bool = False) -> Dict:
    text = md_path.read_text(encoding="utf-8")
    ...
    try:
        ...
        canonical_h1 = title_from_filename
        
        mismatch = parsed_h1 != canonical_h1 and parsed_h1 != canonical_h1.replace("Z1 - ", "").strip()
        if mismatch:
            if fix_titles:
                changed = rewrite_h1_in_markdown(md_path, canonical_h1)
                if changed:
                    print(f"FIXED: {md_path.name}: H1 rewritten '{canonical_h1}'")
                # After rewrite, continue normalization using filename authority (as before)
            else:
                print(f"WARNING: {md_path.name}: H1 '{parsed_h1}' does not match filename '{canonical_h1}'")
...
def main() -> int:
    ...
    ap.add_argument(
        "--fix-titles",
        action="store_true",
        help="Rewrite Markdown H1 to match filename stem (e.g., '# Z1 - Behind House').",
        )
    ...
    for md in md_files:
        try:
            obj = normalize_room_markdown(md, fix_titles=args.fix_titles)
